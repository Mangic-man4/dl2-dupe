...
def sync_pickup():
	...
	lag_delay = 0.12       # extra wait for non-hosts
	...
	print(f"Armed! Will fire at {target_time.strftime('%H:%M:%S')} ms, with {int(lag_delay*1000)} ms lag"
		  f"(in {wait_time:.2f}s).")

	time.sleep(wait_time + lag_delay)

	keyboard.press_and_release(pickup_key)
	print("Pressed, press f8 again to trigger again.")
	return
